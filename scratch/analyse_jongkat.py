...
def main():
    ...
    # Map PPL to SLS for each PML in Jongkat
    jongkat_pmls = ["Dandy", "Novi Sri Wulandari", "Prabowo", "Syarifah Desti Pratiwi", "Vika Rizkiani"]
    pml_to_sls = {pml: [] for pml in jongkat_pmls}
    pml_ppl_to_sls = {} # (pml, ppl): list of sls
    pml_to_pj = {}

    for pj, pmls in pj_kuda_groups.items():
        for pml, ppls in pmls.items():
            if pml in jongkat_pmls:
                pml_to_pj[pml] = pj
                for ppl, sls_list in ppls.items():
                    pml_to_sls[pml].extend(sls_list)
                    pml_ppl_to_sls.setdefault((pml, ppl), []).extend(sls_list)

    # Calculate metrics for Jongkat PMLs
    pml_metrics = {}
    for pml in jongkat_pmls:
    ...
    # Let's print Table 1
    print("\nTABEL 1: PERBANDINGAN MAKRO KINERJA PML KECAMATAN JONGKAT")
    print("-" * 155)
    print(f"{'Dimensi Perbandingan':<25} | {'Dandy':<15} | {'Novi Sri W.':<15} | {'Prabowo':<15} | {'Syarifah D. P.':<15} | {'Vika Rizkiani':<15} | {'Rata-rata Kab':<15}")
    print("-" * 155)
    ...
    def print_row(label, key, is_pct=False, is_daily=False, is_plain_int=False):
        ...
        if key == "target":
            ...
            avg_val = "-"
        print(f"{label:<25} | {vals[0]:<15} | {vals[1]:<15} | {vals[2]:<15} | {vals[3]:<15} | {vals[4]:<15} | {avg_val:<15}")
    ...
    print(f"{'Status Bottleneck':<25} | {bottleneck_strs[0]:<15} | {bottleneck_strs[1]:<15} | {bottleneck_strs[2]:<15} | {bottleneck_strs[3]:<15} | {bottleneck_strs[4]:<15} | -")
    print("-" * 155)
