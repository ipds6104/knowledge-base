...
// Ditujukan kepada Camat Kecamatan Anjongan
...
#grid(
  ...
  [
    #grid(
      ...
      gutter: 3pt,
      [Nomor], [:], [B-1099/61046/HM.310/2026],
      [Sifat], [:], [Biasa],
...
Sehubungan dengan penyusunan Publikasi Kecamatan Anjongan Dalam Angka 2026, kami bermaksud mengajukan permohonan data nama camat, kepala desa/lurah, kepala dusun, serta profil kepegawaian aparatur sipil negara di lingkungan Pemerintah Kecamatan Anjongan sesuai format terlampir.

Pengisian data dapat dilakukan secara daring melalui tautan lembar kerja berikut: #link("https://s.bps.go.id/kcda26-anjongan")[#text(fill: rgb("#0055D4"), weight: "bold")[https://s.bps.go.id/kcda26-anjongan]]. Besar harapan kami data tersebut dapat kami terima selambat-lambatnya pada *Senin, 21 September 2026*. Apabila memerlukan koordinasi lebih lanjut, Bapak/Ibu dapat menghubungi narahubung kami, *Sukma Andini, S.Tr.Stat.* (WhatsApp: *082234120921*).
...
#v(8pt)
#align(center)[
  #text(11pt, weight: "bold")[Lembar Konfirmasi dan Pemutakhiran Data Sektoral \ Kecamatan Anjongan Dalam Angka 2026]
]
#v(6pt)
...
#rect(fill: rgb("#EFF6FF"), stroke: 0.6pt + rgb("#93C5FD"), radius: 3pt, inset: (x: 10pt, y: 6pt))[
  #text(8.5pt)[
    *Petunjuk Pengisian & Konfirmasi Data:*
    ...
    + Pengisian atau konfirmasi data kondisi terkini dilakukan langsung melalui lembar kerja online (*Google Sheets*) pada tautan: #link("https://s.bps.go.id/kcda26-anjongan")[#text(fill: rgb("#0055D4"), weight: "bold")[https://s.bps.go.id/kcda26-anjongan]].
...
#table(
  ...
  stroke: (col, row) => {
    ...
    let top_b = if row == 0 { 0.8pt + black } else if row == 8 { red_b } else { norm }
    let bot_b = if row == 0 { 1.2pt + black } else if row == 8 { red_b } else { norm }
    let left_b = if row == 8 and col == 0 { red_b } else { norm }
    let right_b = if row == 8 and col == 2 { red_b } else { norm }
  ...
  [7], [Ferry Ramadhani, S.STP, MAP], [2022 –2025],
  [8], [*Camat Baru* #text(size: 7.5pt, fill: rgb("#DC2626"))[\ (Tuliskan nama camat baru di sini jika ada pergantian jabatan)]], [],
)
...
#table(
  ...
  [1], [Anjungan Melancar], [Iskak, SE], [],
  [2], [Anjungan Dalam], [Andi Wiyata, SH.], [],
  [3], [Pak Bulu], [Sumadi], [],
  [4], [Dema], [Herkulanus Endi], [],
  [5], [Kepayang], [Mardianus], [],
)
...
#table(
  ...
  stroke: (col, row) => {
    ...
    let bot_b = if row == 0 { 1.2pt + black } else if row == 12 { if col == 4 { red_b } else { norm } } else { norm }
    (top: top_b, bottom: bot_b, left: left_b, right: right_b)
  },
  fill: (col, row) => if row == 0 { rgb("#F3F4F6") } else { none },
  table.header(
  ...
  [1], [Anjungan Melancar], [–], [–], [],
  [2], [Anjungan Dalam], [1. Terdu], [Margareta], [],
  [], [], [2. Pramuka], [Basuni], [],
  [3], [Pak Bulu], [1. Perikanan], [Satibi], [],
  [], [], [2. Pak Bulu], [Sakamin], [],
  [4], [Dema], [1. Dema], [Karlos], [],
  [], [], [2. Beguruh], [Mundet], [],
  [], [], [3. Pinyuh Kersik], [Andreas Atin], [],
  [5], [Kepayang], [1. Bilado], [Fransiskus], [],
  [], [], [2. Lahuak], [Supiri], [],
  [], [], [3. Peladis], [Marsianus], [],
  [], [], [4. Kepayang], [Robertus], [],
)
...
#table(
  ...
  [1], [Pemerintah Daerah Kecamatan Anjongan], [], [], [],
  [2], [Anjungan Melancar], [], [], [],
  [3], [Anjungan Dalam], [], [], [],
  [4], [Pak Bulu], [], [], [],
  [5], [Dema], [], [], [],
