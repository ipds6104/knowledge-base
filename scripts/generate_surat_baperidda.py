#!/usr/bin/env python3
"""
generate_surat_baperidda.py
===========================
Script untuk menghasilkan Surat Dinas Resmi BPS Kabupaten Mempawah kepada Baperidda:
Nomor: B-1084/61046/HM.310/2026
Hal: Permintaan Data Untuk Publikasi Kecamatan Dalam Angka 2026 (Luas Wilayah 2025)

Revisi Kak Sukma:
1. Di bawah nama "Munawir" dihapus (hanya nama saja).
2. Font teks: "Arial", ukuran font isi surat: 12pt.
3. Halaman kedua sampai akhir diberi header logo BPS dan SE2026 disamakan seperti halaman 1.
4. Judul lampiran dibuat simpel sesuai screenshot (Lampiran 1, Nomor, Tanggal rata kiri).
"""
...
OUTPUT_DIR = REPO_ROOT / "kegiatan" / "kecamatan-dalam-angka" / "2026" / "outputs" / "surat"
...
def build_typst_content() -> str:
    # Build table rows
    table_rows = []
    # Header row
    table_rows.append('    table.header([*No*], [*Kecamatan*], [*Desa/Kelurahan*], [*Luas Total Area (km#super[2])*], [*(1)*], [*(2)*], [*(3)*], [*(4)*]),')
    
    for no, kec, desa in TABLE_DATA:
        kec_display = f"[*{kec}*]" if kec else "[]"
        table_rows.append(f'    [{no}], {kec_display}, [{desa}], [],')
    ...
    content = f'''// Surat Dinas Permintaan Data KCDA 2026 ke Baperidda Kabupaten Mempawah
#set page(
  paper: "a4",
  margin: (
    top: 4.4cm,
...
#table(
  ...
  stroke: (col, row) => {{
    ...
    let bot_b = if row == 1 {{ 1pt + black }} else if row == {total_rows} {{ 1.2pt + black }} else {{ norm }}
    
    // Border merah outer pada kolom isian (kolom 3 / Luas Total Area)
    let is_val_row = (row >= 2 and row < {total_rows})
    let r_top = if row == 2 and col == 3 {{ red_b }} else {{ top_b }}
    let r_bot = if row == {total_rows - 1} and col == 3 {{ red_b }} else {{ bot_b }}
...
def main():
    typ_path = OUTPUT_DIR / "surat-permintaan-kcda-2026-baperidda.typ"
    pdf_path = OUTPUT_DIR / "surat-permintaan-kcda-2026-baperidda.pdf"
    
    print("📝 Menyusun berkas Typst...")
    content = build_typst_content()
    typ_path.write_text(content, encoding="utf-8")
    
    print("⚙️ Mengompilasi Typst ke PDF...")
    cmd = [
    ...
    if res.returncode != 0:
        print(f"❌ Kompilasi gagal: {res.stderr}")
        sys.exit(1)
    
    print(f"✅ Berhasil dikompilasi: {pdf_path} ({pdf_path.stat().st_size / 1024:.1f} KB)")
